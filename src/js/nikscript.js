...
function parser(tokens, index, type, returnsymbol) {
    ...
    while (token[0] != returnsymbol) {
        ...
        if (type == "assignment" && (token[0] == "<" || token[0] == ">")) { // special case used in for loops
            ...
            var data = parser(tokens, index, "operation", ";")
            ...
            var data = parser(tokens, index + 1, "operation", ";")
            ...
            if (token[1] == "var") {
                var data = parser(tokens, index + 1, "assignment", ";")
                ...
                if (tokens[index + 2][0] == "=") {
                    if (type == "condition") {
                        ...
                        var data = parser(tokens, index, "comparison", ")")
                    ...
                    var data = parser(tokens, index, "assignment", ";")
                    result.push(["assignment", data[0]])
                    index = data[1] - 1
                }
            }
                ...
                var data = parser(tokens, index + 2, "call", ")")
            ...
            var data = parser(tokens, index + 1, "bracket", ")")
        ...
        if (expr[0] == "number") {
            ...
            if (content[0] + "()" in functions) { // functioncall for selvedefined function
                ...
                if (argsGiven.length == argsNeeded.length) {
                    ...
                    var returnValue = functionStack[functionStack.length - 1][2]
                    ...
                    if (outputObject == undefined)
                        ...
                        outputObject.current += content[0] + " takes " + argsNeeded.length + " arguments but " + argsGiven.length + " were given!"
                }
            }
                ...
                switch (content[0]) {
                    case "print":
                        if (outputObject == undefined)
                            ...
                            outputObject.current += interpreter(content[1]) + "\n"
                        break
                    ...
                    default:
                        if (outputObject == undefined)
                            ...
                            outputObject.current += "function \"" + content[0] + "\" undefined"
                }
            }
        }
...
export default function interpret(code, output) {
    if (code.length > 0) {
        outputObject = output
        if (outputObject != undefined) {
            outputObject.current = ""
        }
